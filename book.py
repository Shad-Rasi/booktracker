...
def detailseite(b_id: int):
    ...
    def inline_speichern(neuer_status=None, neuer_rating_wert=None):
        ...
        # --- DER AUTOMATISCHE DATUMS-SICHERHEITSGURT ---
        ...
        sterne_bereich_refresh.refresh()
            ...
            with ui.element('div').classes('w-full md:w-1/3 flex flex-col gap-4'):
                ...
                with ui.dialog() as search_dialog, ui.card().classes('p-6 w-full max-w-2xl flex flex-col gap-4').style(style_dialog_card):
                    ...
                    async def suche_ausfuehren():
                        ...
                        if not urls:
                            with galerie_container:
                                ui.label(t('book_no_images_found')).classes('text-center text-slate-500 dark:text-slate-400 my-4')
                            return
                ...
                with ui.dialog() as upload_dialog, ui.card().classes('p-6 w-96 flex flex-col gap-4').style(style_dialog_card):
                    ...
                    ui.upload(
                        label=t('book_select_image_label'),
                        auto_upload=True,
                ...
                with ui.row().classes('w-full justify-center gap-2'):
                    ui.button(t('book_btn_change_cover'), icon='photo_camera', on_click=upload_dialog.open) \
                        .props('flat dense').classes('text-blue-600 dark:text-blue-400 text-xs')
                    
                    ui.button(t('book_btn_search_online'), icon='search', on_click=lambda: [search_dialog.open(), asyncio.create_task(suche_ausfuehren())]) \
                ...
                def infokarte_links_refresh():
                    ...
                    with ui.card().classes(f'w-full p-4 shadow-sm flex flex-col gap-2.5 text-sm {bg_infokarte}'):
                        ...
                        ui.select(options=status_opts, value=seiten_status['aktueller_status'], on_change=lambda e: inline_speichern(neuer_status=e.value))\
                            .classes('w-full px-2 py-1 rounded bg-slate-100 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-700 text-xs mt-1').props(f'dense borderless {dark_prop}')
                        ...
                        if location_name:
                            ui.label(f"{t('location')}: {location_name}")
            ...
            with ui.element('div').classes('flex-1 flex flex-col gap-4 w-full'):
                ...
                with ui.row().classes('w-full justify-between items-start no-wrap gap-4'):
                    ...
                    with ui.element('div').classes('flex flex-col gap-1 flex-1 items-start'):
                        ...
                        if author and author_id:
                            ui.label(author) \
                                .classes('text-xl text-slate-600 dark:text-slate-300 italic mt-1 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 hover:underline transition-colors inline-block') \
                    ...
                    if current_status == 'READING':
                        ...
                        with ui.card().classes(f'w-full p-4 shadow-sm mb-4 {bg_card}'):
                            ...
                            with ui.row().classes('w-full items-center gap-3 no-wrap mb-4'):
                                seiten_input = ui.number(label=t('book_input_new_page'), value=letzte_seite + 1, min=0, max=gesamt_seiten) \
                                ...
                                for index, (cyc_id, s_at, f_at, r_cycle) in enumerate(beendete_zyklen, 1):
                                    with ui.row().classes(f'w-full justify-between items-center text-xs p-2.5 rounded border shadow-xs no-wrap gap-2 {bg_log_row}'):
                                        with ui.element('div').classes('flex flex-col gap-0.5'):
                                            ...
                                            ui.label(f"🗓️ {translations.format_localized_date(s_at) or '--'} {t('book_until')} {translations.format_localized_date(f_at) or '--'}").classes(f'font-mono text-[11px] {text_sub}')
                ...
                def globales_logbuch_refresh():
                    ...
                    with ui.expansion(t('book_view_logbook'), icon='auto_stories').classes('w-full border rounded-lg bg-slate-100/30 dark:bg-slate-900/10 dark:border-slate-700/50 mt-4'):
                        if not alle_logs:
                            ui.label(t('book_no_logs_recorded')).classes(f'text-xs italic p-4 text-center {text_sub}')
                        else:
