...
import asyncio
import book_api  # WICHTIG: Sicherstellen, dass book_api importiert ist!
...
def detailseite(b_id: int):
    ...
    # --- LIVE SPEICHERFUNKTION (KORRIGIERT) ---
    def inline_speichern():
        ...
        # Aktualisiert beide Bereiche sauber im Hintergrund
    ...
    with basis_layout(title):
        ...
        with ui.element('div').classes('w-full max-w-4xl mx-auto flex flex-col md:flex-row gap-8 items-start'):
            ...
            with ui.element('div').classes('w-full md:w-1/3 flex flex-col gap-4'):
                ...
                with ui.element('div').classes('relative w-full aspect-[2/3] rounded-lg shadow-md border bg-slate-200 flex items-center justify-center overflow-hidden') as cover_container:
                    if cover_pfad != "/covers/placeholder.jpg":
                        ui.image(cover_pfad).classes('w-full h-full object-cover')
                    else:
                        with ui.element('div').classes('flex flex-col items-center gap-2 text-slate-400'):
                ...
                with ui.dialog() as search_dialog, ui.card().classes('p-6 w-full max-w-2xl flex flex-col gap-4'):
                    ui.label('Cover im Internet suchen').classes('text-lg font-bold text-slate-700')
                    ui.label('Wähle ein passendes Cover aus der Live-Suche aus:').classes('text-xs text-slate-500 -mt-2')
                    ...
                    async def bild_waehlen(url):
                        ...
                        erfolg = database.lade_and_speichere_cover(book_id, url)
                        if erfolg:
                    ...
                    async def suche_ausfuehren():
                        ...
                        urls = await book_api.google_bildsuche_async(title, author, isbn=such_isbn)
                        
                        galerie_container.clear()
                        
                        if not urls:
                            with galerie_container:
                                ui.label('Keine passenden Bilder gefunden.').classes('text-center text-slate-500 my-4')
                            return
                            
                        with galerie_container:
                            with ui.grid(columns=4).classes('gap-4 w-full justify-items-center max-h-96 overflow-y-auto p-1'):
                                for url in urls:
                                    with ui.element('div').classes('border border-slate-200 rounded p-1 hover:shadow-md hover:border-blue-500 cursor-pointer transition-all bg-white'):
                                        img = ui.image(url).classes('w-24 h-36 object-cover')
                                        img.on('click', lambda e, u=url: bild_waehlen(u))
                    
                    ui.button(t('cancel'), on_click=search_dialog.close).classes('bg-slate-400 text-white w-full mt-2')

                # --- DIALOG FÜR MANUELLEN UPLOAD ---
                with ui.dialog() as upload_dialog, ui.card().classes('p-6 w-96 flex flex-col gap-4'):
                    ui.label('Cover hochladen / ändern').classes('text-lg font-bold text-slate-700')
                    ...
                    ui.button(t('cancel'), on_click=upload_dialog.close).classes('bg-slate-400 text-white w-full')
                ...
                with ui.row().classes('w-full justify-center gap-2'):
                    ui.button('Cover ändern', icon='photo_camera', on_click=upload_dialog.open) \
                        .props('flat dense').classes('text-blue-600 text-xs')
                    
                    ui.button('Online suchen', icon='search', on_click=lambda: [search_dialog.open(), asyncio.create_task(suche_ausfuehren())]) \
                        .props('flat dense').classes('text-emerald-600 text-xs')

                # --- JETZT REAKTIV: Linke Infokarte ---
                @ui.refreshable
                def infokarte_links_refresh():
                    with ui.card().classes('w-full p-4 bg-slate-50 border border-slate-200 shadow-sm flex flex-col gap-2.5 text-sm text-slate-700'):
                        ...
                        # Holt die Zeiten (mit dem neuen Fallback!)
                        ...
                        if real_start or real_end:
                            ...
                            with ui.element('div').classes('flex flex-col gap-1 text-xs text-slate-500 bg-white p-2 rounded border border-slate-100 shadow-inner'):
                                if cyc_status == 'READING':
                                    ui.label('⚡ Aktueller Durchgang:').classes('font-bold text-emerald-600')
                                    ui.label(f"Gestartet am: {translations.format_localized_date(real_start) or '--'}")
                                elif cyc_status == 'READ':
                                    ui.label('✅ Zuletzt gelesen:').classes('font-bold text-blue-600')
                ...
                # Initialer Aufruf der linken Karte
            ...
            with ui.element('div').classes('flex-1 flex flex-col gap-4 w-full'):
                ...
                # --- TRACKING-ZONE (FORTSCHRITTSBALKEN, HISTORIE & REREADS) ---
                @ui.refreshable
                def tracking_zone_refresh():
                    current_status = status_select.value
                    
                    # --- COMPONENT 1: AKTUELLES TRACKING ODER REREAD-BUTTON ---
                    if current_status == 'READING':
                        ...
                        with ui.card().classes('w-full p-4 bg-slate-50 border border-slate-200 shadow-sm mb-4'):
                            ui.label('Lese-Fortschritt tracken 📈').classes('text-sm font-bold text-slate-700 mb-1')
                            
                            ui.linear_progress(value=prozent, color='emerald', show_value=False).classes('w-full my-2 h-2 rounded')
                            ui.label(f"Aktuell auf Seite {letzte_seite} von {gesamt_seiten} ({int(prozent*100)}%)").classes('text-xs text-slate-500 mb-3')
                            ...
                            with ui.row().classes('w-full items-center gap-3 no-wrap mb-4'):
                                seiten_input = ui.number(label='Neue Seite', value=letzte_seite + 1, min=0, max=gesamt_seiten) \
                                    .props('dense outlined').classes('w-32 bg-white')
                                ...
                                async def speichern_klick():
                                    ...
                                    if erfolg:
                                        ...
                                        if neuer_stand >= gesamt_seiten:
                                            # --- NEU: Erst den Zyklus in der DB sauber schließen ---
                                            database.schliesse_aktiven_zyklus_ab(layout.aktiver_user_id, book_id)
                                            
                                            # Dann den globalen Buchstatus ändern
                            ...
                            # PROTOKOLL-LISTE MIT LÖSCH-OPTION
                            if logs:
                                ui.separator().classes('my-2')
                                ui.label('Logbuch dieses Durchgangs:').classes('text-xs font-bold text-slate-400 uppercase tracking-wider mb-2')
                                with ui.element('div').classes('w-full max-h-40 overflow-y-auto flex flex-col gap-1.5'):
                                    for l_date, p_page, p_read in logs:
                                        with ui.row().classes('w-full justify-between items-center text-xs bg-white p-2 rounded border border-slate-100 shadow-sm no-wrap gap-2'):
                                            with ui.row().classes('items-center gap-3'):
                                                ui.label(translations.format_localized_date(l_date)).classes('font-medium text-slate-500 font-mono')
                                                ui.label(f"S. {p_page} (gelesen: +{p_read})").classes('text-slate-700 font-semibold')
                        ...
                        with ui.card().classes('w-full p-4 bg-emerald-50/40 border border-emerald-100 flex flex-col gap-2 items-center text-center mb-4'):
                            ui.icon('auto_stories', size='md').classes('text-emerald-600')
                            ui.label('Du hast dieses Buch bereits ausgelesen!').classes('text-sm font-bold text-slate-700')
                            ui.label('Möchtest du es noch einmal von vorne lesen und einen neuen Durchgang starten?').classes('text-xs text-slate-500 max-w-sm')
                        ...
                        with ui.element('div').classes('p-3 bg-slate-50 rounded border border-dashed text-center text-xs text-slate-400 italic mb-4'):
                            ui.label('Stelle den Status auf "Am Lesen", um deinen Lesefortschritt zu dokumentieren.')

                    # --- COMPONENT 2: ANZEIGE ALLER VERGANGENEN REREADS / ZYKLEN ---
                    beendete_zyklen = database.lade_beendete_zyklen(layout.aktiver_user_id, book_id)
                    if beendete_zyklen:
                        with ui.card().classes('w-full p-4 bg-blue-50/30 border border-blue-100/70 shadow-sm'):
                            with ui.row().classes('items-center gap-2 mb-1'):
                                ui.icon('history', size='sm').classes('text-blue-500')
                                ui.label('Vergangene Lesedurchgänge').classes('text-xs font-bold text-slate-500 uppercase tracking-wider')
                            ...
                            with ui.element('div').classes('w-full flex flex-col gap-2 mt-1'):
                                for index, (cyc_id, s_at, f_at, r_cycle) in enumerate(beendete_zyklen, 1):
                                    durchgang_nummer = len(beendete_zyklen) - index + 1
                                    
                                    with ui.row().classes('w-full justify-between items-center text-xs bg-white p-2.5 rounded border border-slate-100 shadow-xs no-wrap gap-2'):
                                        with ui.element('div').classes('flex flex-col gap-0.5'):
                                            ui.label(f"{index}. Lesedurchgang").classes('font-bold text-slate-700')
                                            ui.label(f"🗓️ {translations.format_localized_date(s_at) or '--'} bis {translations.format_localized_date(f_at) or '--'}").classes('text-slate-500 font-mono text-[11px]')
                                        ...
                                        with ui.row().classes('items-center gap-3 shrink-0'):
                                            ...
                                            async def zyklus_loeschen(target_id=cyc_id):
                                                with ui.dialog() as confirm_dial, ui.card().classes('p-4'):
                                                    ui.label('Diesen Lesedurchgang wirklich löschen? Alle damit verknüpften täglichen Kalendereinträge gehen verloren!').classes('text-sm text-slate-600 mb-4')
                ...
                # Initiales Zeichnen der gesamten Tracking-Zone
