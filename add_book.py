...
def formular_rendern(edit_id=None, daten=None):
    ...
    def speichern():
        ...
        if not title:
            ui.notify(t('title_missing'), type='warning')
            ui_log_lang('log_title_missing')
            return
        ...
        ui_log_lang('log_book_saved', title = title)
    ...
    # =========================================================================
    # REPARIERT & DYNAMISIERT: DIE NEUE KASKADIERENDE SUCHE
    # =========================================================================
    async def isbn_suchen():
        ...
        ui.notify(t('import_preparing'), type='info')
        
        # 1. Bevorzugte Prioritätenliste des Nutzers aus der DB laden
        ...
        # 2. Den kaskadierenden Master-Router triggern
        vereinte_daten = await book_api.hole_buch_details_kaskadierend(isbn_wert, provider_reihenfolge)
        
        if vereinte_daten and (vereinte_daten.get('title') or vereinte_daten.get('series_name')):
            # Felder im UI befüllen
            ...
            isbn_10_input.value = vereinte_daten.get('isbn_10', '')
            ...
            # Reihen-Logik automatisch mappen (wenn der Provider sie geliefert hat)
            ...
            # Erfolg im Logfenster protokollieren!
            ui_log_lang('log_api_success', quelle=vereinte_daten.get('quelle', 'Unknown'), title=vereinte_daten.get('title', ''))
            ui.notify(t('notify_api_success'), type='positive')
        else:
            ui_log_lang('log_api_fail', isbn=isbn_wert)
