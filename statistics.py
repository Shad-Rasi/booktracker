...
def berechne_zeitliche_statistiken(buecher, ziel_jahr):
    """Berechnet dynamisch die persönlichen Daten – gesplittet nach Jahr oder ALL."""
    total_pages_read = 0
...
def statistik_seite():
    ...
    with basis_layout('stats_title'):
        # OPTIMIERT: Padding-Wrapper für Mobile gegen Display-Anklatschen
        with ui.element('div').classes('w-full px-4 md:px-0 content-safe mb-12'):
            ui.label(t('stats_title')).classes('text-2xl font-bold text-slate-700 dark:text-slate-100 mb-1')
            ...
            with ui.dialog() as share_modal, ui.card().classes('p-6 w-full max-w-sm').style(style_modal):
            ...
            # =========================================================================
            with ui.row().classes('w-full justify-between items-center mb-4 gap-2 flex-wrap sm:flex-nowrap'):
                ui.label(t('stats_sec_personal')).classes('text-lg font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wide')
                
                # Optimiert: Dropdowns brechen mobil sauber um und nutzen flex-1
                with ui.row().classes('gap-2 items-center w-full sm:w-auto justify-end'):
                    ...
                    ).classes('flex-1 sm:w-40 px-1 text-sm') \
                    ...
                    auswahl_jahr = ui.select(
                        ...
                        value=aktuelles_jahr,  
                        on_change=lambda: personal_stats_container.refresh()
                    ).classes('flex-1 sm:w-36 px-1 text-sm') \
                     .props(f'dense outlined {dark_prop}')
            ...
            def personal_stats_container():
                ...
                # Verlaufscharts (Optimiert: Flexiblere Höhen via h-64 sm:h-80)
                with ui.element('div').classes('w-full grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'):
                    with ui.card().classes(f'p-4 w-full h-64 sm:h-80 items-center border shadow-sm {bg_card}'):
                        ui.label(t('stats_chart_books_development')).classes('text-sm font-bold self-start text-slate-600 dark:text-slate-300')
                        ...
                        ui.echart({
                            ...
                            'grid': {'top': '15%', 'left': '8%', 'right': '5%', 'bottom': '15%'}, # Mehr Platz für Achsen mobil
                            'xAxis': {'type': 'category', 'data': p_stats['x_achse_keys'], 'axisLabel': {'color': axis_text_color, 'fontSize': 10}},
            ...
            # =========================================================================
            # SEKTION 2: ALLGEMEINE TRACKER STATISTIKEN (REPARIERT)
            # =========================================================================
            ...
            # Raster: Mobil 1 Spalte, ab md: (Desktop) 3 Spalten
            with ui.element('div').classes('w-full grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'):
                
                # 1. KPI Karte (Bestandsliste)
                with ui.card().classes(f'p-4 border-l-4 border-blue-500 shadow-sm flex flex-col justify-between h-auto md:min-h-[19rem] {bg_kpi_card}'):
                    with ui.element('div'):
                    ...
                    with ui.element('div').classes('w-full flex flex-col gap-1.5 pt-2 border-t border-slate-200 dark:border-slate-700 text-xs mt-2 md:mt-0'):
                ...
                # 2. Ownership Kuchendiagramm
                with ui.card().classes(f'p-4 h-72 md:min-h-[19rem] flex flex-col justify-between border shadow-sm {bg_card}'):
                    label_ownership_chart = t('ownership')
                    ui.label(label_ownership_chart).classes('text-sm font-bold self-start text-slate-600 dark:text-slate-300 mb-1')
                    ...
                    ui.echart({
                        ...
                        'legend': {'type': 'scroll', 'bottom': '0%', 'left': 'center', 'textStyle': {'color': axis_text_color, 'fontSize': 10}},
                        'backgroundColor': 'transparent',
                        'series': [{
                            'name': label_ownership_chart, 
                            'type': 'pie', 
                            'radius': '60%',          
                            'center': ['50%', '42%'], 
                            'label': {'show': False},
                    ...
                    }).classes('w-full h-48 sm:h-52') # REPARIERT: Feste Pixelhöhe zwingt den Kuchen in den Rahmen

                # 3. Buchtyp-Kuchendiagramm (REPARIERT: Steckt jetzt wieder sicher in einer Card!)
                with ui.card().classes(f'p-4 h-72 md:min-h-[19rem] flex flex-col justify-between border shadow-sm {bg_card}'):
                    ui.label(t('stats_chart_formats')).classes('text-sm font-bold self-start text-slate-600 dark:text-slate-300 mb-1')
                    ...
                    }).classes('w-full h-48 sm:h-52') # REPARIERT: Feste Pixelhöhe fängt ECharts ab
            ...
            with ui.element('div').classes('w-full grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 mb-6'):
                ...
                # Buchlängen (Optimiert: Schräge Labels für Mobile, damit "Seiten_unter_200" lesbar bleibt)
                with ui.card().classes(f'p-4 w-full h-64 sm:h-80 border shadow-sm {bg_card}'):
                    ui.label(t('stats_chart_lengths')).classes('text-sm font-bold self-start text-slate-600 dark:text-slate-300 mb-1')
                    
                    ui.echart({
                        'backgroundColor': 'transparent',
                        'grid': {'top': '15%', 'left': '8%', 'right': '5%', 'bottom': '25%'}, # Mehr Bottom-Grid für schräge Texte
                        'xAxis': {
                            'type': 'category', 
                            'data': global_stats['seiten_keys'], 
                            'axisLabel': {'color': axis_text_color, 'fontSize': 9, 'rotate': 20}
                        },
                ...
                # Genre-Verteilung
                with ui.card().classes(f'p-4 w-full h-64 sm:h-80 border shadow-sm {bg_card}'):
                    ui.label(t('stats_chart_genres')).classes('text-sm font-bold self-start text-slate-600 dark:text-slate-300 mb-1')
                    ...
                    if not genre_daten:
                        ...
                        ui.echart({
                            'backgroundColor': 'transparent',
                            'grid': {'top': '15%', 'left': '8%', 'right': '5%', 'bottom': '25%'},
                            'tooltip': {'trigger': 'axis', 'axisPointer': {'type': 'shadow'}},
                            'xAxis': {
                                'type': 'category', 
                                'data': genre_labels, 
                                'axisLabel': {'color': axis_text_color, 'interval': 0, 'rotate': 30, 'fontSize': 9}
                            },
                            'yAxis': {
                                'type': 'value', 
                            ...
                            'series': [{
                                ...
                                'type': 'bar',
                                'data': genre_werte,
                                'label': {'show': True, 'position': 'top', 'color': axis_text_color, 'fontSize': 9},
            ...
            with ui.element('div').classes('w-full grid grid-cols-1 md:grid-cols-2 gap-6'):
                with ui.card().classes(f'p-4 w-full h-64 sm:h-80 items-center border shadow-sm {bg_card}'):
                    ...
                    # Ganz unten bei 'stats_chart_status'
                    ui.echart({
                        'tooltip': {'trigger': 'item'},
                        'legend': {'type': 'scroll', 'bottom': '0%', 'left': 'center', 'textStyle': {'color': axis_text_color, 'fontSize': 10}},
                        'backgroundColor': 'transparent',
                        'series': [{
                            'name': t('books'), 'type': 'pie', 'radius': ['35%', '65%'], 'center': ['50%', '42%'],
                    ...
                    }).classes('w-full h-48 sm:h-52') # REPARIERT: Stabile Höhe statt flex-1
                ...
                with ui.card().classes(f'p-4 w-full h-64 sm:h-80 items-center border shadow-sm {bg_card}'):
                    ...
                    ui.echart({
                        ...
                        'xAxis': {'type': 'category', 'data': global_stats['sterne_keys'], 'axisLabel': {'color': axis_text_color, 'fontSize': 10}},
                        'yAxis': {'type': 'value', 'minInterval': 1, 'axisLabel': {'color': axis_text_color}, 'splitLine': {'lineStyle': {'color': grid_line_color}}},
                        'tooltip': {'trigger': 'axis'},
                        'series': [{
